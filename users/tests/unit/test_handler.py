...
In the test_handler.py script, the test_environment() method injects 
a mock DynamoDB into the environment, then set_up_dynamodb() creates 
a mock Users table, and finally put_data_dynamodb() creates the test 
data.'''
...
def put_data_dynamodb():
    ...
    conn.put_item(
        ...
        Item={
            ...
            'name': {'S':'John Doe'},
            'timestamp': {'S':'2021-03-30T17:13:06.515Z'}
        }
    )
    conn.put_item(
        TableName=USERS_MOCK_TABLE_NAME,
...
def test_get_list_of_users():
    with my_test_environment():
        ...
        expected_response = [
            {
                ...
                'name': 'John Doe',
                'timestamp': '2021-03-30T17:13:06.515Z'
            },
            {
        ...
        assert sorted(data, key=lambda x: x['userid']) == sorted(expected_response, key=lambda x: x['userid'])
